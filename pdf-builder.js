// PDF BUILDER — Atelier A2: Wortbildung
(function () {
    ...
    function buildCast() {
        return `<div class="cast-banner">
            ...
            <div class="cast-grid">
                ...
                <div class="cast-card"><img src="images/andreea.png" alt="Andreea"><div class="name">Andreea 🇷🇴</div><div class="detail">Ghidul tău · Potsdam</div></div>
    ...
    function buildTheory() {
        ...
        return `<h1 class="chapter">📘 1. Teorie — Wortbildung (-ung / -heit / -keit = die · Nominalisierung = das)</h1>` + t;
    }

    // Bază → substantiv (cu traducere RO) — pentru ex de formare (ex1/ex2/ex3)
    function formationBlock(title, instruction, data, baseField, baseLabel) {
        let h = `<div class="ex-block"><h3>${title}</h3><div class="instruction">${instruction}</div><div class="rezolvare-banner">✓ Rezolvare</div>
            <table><thead><tr><th>${baseLabel}</th><th>🇩🇪 Substantiv</th><th>🇷🇴 Traducere</th></tr></thead><tbody>`;
        data.forEach(it => {
            h += `<tr><td>${it[baseField]}</td><td class="verb">${it.correct}</td><td class="ro-text">${it.sens || ''}</td></tr>`;
        });
        h += `</tbody></table></div>`;
        return h;
    }

    // Alege articolul (die/das) — pentru ex4
    function articleChoiceBlock(title, instruction, data) {
        let h = `<div class="ex-block"><h3>${title}</h3><div class="instruction">${instruction}</div><div class="rezolvare-banner">✓ Rezolvare</div>`;
        data.forEach((it, i) => {
            h += `<div class="ex-item"><span class="ex-num">${i + 1}</span><div class="ex-body"><div class="ex-q"><strong style="color:#047857">${it.correct}</strong> ${it.word}</div><div class="ex-explanation">${it.expl}</div></div></div>`;
        });
        return h + `</div>`;
    }

    // Traducere RO → DE — pentru ex5
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += formationBlock('Übung 1 — verb + -ung → die', 'Formează substantivul în -ung (toate sunt feminine → die).', ex1Data, 'verb', '🇩🇪 Verb');
        }
        if (typeof ex2Data !== 'undefined') {
            html += formationBlock('Übung 2 — adjectiv + -heit / -keit → die', 'Adjective scurte → -heit · adjective în -ig/-lich/-bar → -keit. Toate → die.', ex2Data, 'adj', '🇩🇪 Adjectiv');
        }
        if (typeof ex3Data !== 'undefined') {
            html += formationBlock('Übung 3 — Nominalisierung (das + infinitiv)', 'Infinitivul întreg + literă mare + articolul das.', ex3Data, 'verb', '🇩🇪 Verb');
        }
        if (typeof ex4Data !== 'undefined') {
            html += articleChoiceBlock('Übung 4 — Ce articol? die / das', 'Scurtătura: -ung/-heit/-keit → die · infinitiv nominalizat → das.', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += translateBlock('Übung 5 — Traducere RO → DE', 'Scrie substantivul cu articolul corect (die sau das).', ex5Data);
        }
        return html;
    }
    ...
    function buildVerbs() {
        let html = `<h1 class="chapter new-section">🔁 4. Verbele de bază care formează substantive în -ung — Präsens · Präteritum · Perfekt</h1>
            <div class="andreea-note">
                <div class="andreea-note-content">
                    <div class="speaker">📌 Andreea îți spune:</div>
                    <div>Dintr-un verb scoți un substantiv lipind -ung la radical: wohnen → die Wohnung, üben → die Übung. TOATE substantivele în -ung sunt feminine (die). Așa înveți două cuvinte deodată!</div>
                </div>
            </div>`;
